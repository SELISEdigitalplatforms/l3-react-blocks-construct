...
export interface InvoicesDataTableProps<TData, TValue> {
  /**
  ...
  /**
   * The current search query to filter the table globally.
   */
  searchQuery?: string;
  /**
  ...
  /**
   * Callback function to provide the table instance to the parent.
   */
  onTableInstanceReady?: (table: TableInstance<TData>) => void;
}
...
}: Readonly<InvoicesDataTableProps<TData, TValue>>) {
                ...
                {table.getHeaderGroups().map((headerGroup) => (
                  <TableRow key={headerGroup.id} className="hover:bg-transparent">
                    {headerGroup.headers.map((header) => (
                      <TableHead key={header.id} colSpan={header.colSpan} className="pr-0">
